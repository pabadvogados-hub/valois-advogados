"""
=============================================================================
  FECHAMENTO MENSAL COMPLETO - WELINGTON VALOIS ADVOGADOS ASSOCIADOS
=============================================================================
...
HEADER_FONT_WHITE = Font(name='Segoe UI', bold=True, size=11, color='FFFFFF')
...
def banner(nome_mes, ano, start_date, end_date):
    print('\n')
    print('=' * 70)
    print('   WELINGTON VALOIS ADVOGADOS ASSOCIADOS - FECHAMENTO MENSAL COMPLETO')
...
def passo1_fechamento_asaas(start_date, end_date, mes, ano, drive):
    ...
    ws['A3'] = 'RESUMO DO MES'
    ws['A3'].font = Font(name='Segoe UI', bold=True, size=12)
    for r, (label, val) in enumerate([
        ...
        ws.cell(row=r, column=1, value=label).font = Font(name='Segoe UI', bold=True, size=10)
    ...
    ws.cell(row=linha_resumo, column=4, value='ESCRITORIO:').font = Font(name='Segoe UI', bold=True, size=10)
    ...
    for chave, regra in regras.COMISSOES.items():
        ...
        ws.cell(row=linha_resumo, column=4, value=f"{regra.get('rotulo', chave)}:").font = Font(name='Segoe UI', bold=True, size=10)
    ...
    for chave, regra in regras.COMISSOES.items():
        ...
        ws_c['A1'].font = Font(name='Segoe UI', bold=True, size=13)
        ws_c['B3'] = f'Periodo de 01/{mes}/{ano} ate {end_date[8:]}/{mes}/{ano}'
        ws_c['B3'].font = Font(name='Segoe UI', bold=True, size=10)
...
def passo5_upload_drive(wb_fechamento, fechamento, advbox_data, analise, nome_mes, mes, ano, drive):
    ...
    for chave, info in fechamento['totais_por_comissionado'].items():
        ...
        for d in info['processos']:
            ...
            cr += 1
        cr += 1
        ws_c.cell(row=cr, column=2, value='TOTAL COMISSAO:').font = Font(name='Segoe UI', bold=True, size=12)
        ws_c.cell(row=cr, column=4, value=info['total']).number_format = MONEY_FMT
        ws_c.cell(row=cr, column=4).font = Font(name='Segoe UI', bold=True, size=12)
    ...
    ws_res['A1'].font = Font(name='Segoe UI', bold=True, size=14)
    dados_res = [
    ...
    for i, (label, val) in enumerate(dados_res, 3):
        ws_res.cell(row=i, column=1, value=label).font = Font(name='Segoe UI', bold=True, size=11)
...
def main():
    parser = argparse.ArgumentParser(description='Fechamento Mensal Completo - Welington Valois Advogados Associados')
