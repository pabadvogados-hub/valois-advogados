# Agente Operacional VALOIS_ADVOGADOS (VALOIS.IA)

Agente autonomo que recebe tarefas do ADVBOX (atribuidas ao usuario-agente **VALOIS.IA**) via webhook disparado pela N8N, produz o resultado (peca juridica, notificacao de cliente, resposta operacional) e devolve a tarefa para quem pediu com o link do documento.
...
Equipe do escritorio
   |
   v  (atribui tarefa a VALOIS.IA no ADVBOX)
...
AGENTE_OP_PORT=8787
ADVBOX_USER_AGENTE=<id do usuario-agente VALOIS.IA no ADVBOX>
ADVBOX_TASK_TYPE_ACOMPANHAMENTO=<id do tipo de tarefa para retornos>
ESCRITORIO_ASSINATURA=Dr. Welington Dias Valois - OAB/ES 34.912 - Iúna/ES
...
- **So VALOIS.IA pendente** -> trocar `SEU_ADVBOX_USER_AGENTE` pelo ID do usuario-agente no ADVBOX
...
- Retorno no ADVBOX sempre com `from = usuario-agente (VALOIS.IA)`
