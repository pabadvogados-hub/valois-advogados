...
Fecha a tarefa no ADVBOX:
  ...
  - From = AGENTE_IA_ID (retorno aparece em nome da propria VALOIS.IA)
  - Em caso de sucesso, grava um marcador maquina-legivel ([VALOISIA-FEITO #id])
    que permite a VALOIS.IA detectar, em varreduras futuras, que a tarefa ja foi
...
def _marcador_feito(task_id) -> str:
    """Marcador embutido no comentario de retorno (ex.: '[VALOISIA-FEITO #12345]')."""
...
def ja_concluida_no_advbox(task_id, lawsuit_id) -> bool:
    ...
    Antes de (re)executar uma tarefa, a VALOIS.IA varre os andamentos/publicacoes do
    processo. Se encontrar o marcador de conclusao que ela mesma deixou referenciando
    esta tarefa original (ex.: "[VALOISIA-FEITO #12345]"), retorna True -> nao refazer.
    ...
    for p in posts:
        if alvo.search(_texto_post(p)):
            log.info(f'[{task_id}] marcador de conclusao encontrado no processo '
                     f'{lawsuit_id} - tarefa ja feita pela VALOIS.IA, nao sera refeita')
            return True
    return False
...
def entregar_resultado(task_id, destinatarios, lawsuit_id, resultado: dict):
    ...
    if sucesso:
        # Marcador maquina-legivel: assinala que ESTA tarefa ja foi feita pela VALOIS.IA.
    ...
    try:
        criar_publicacao(
            ...
            comments=comments,
            from_id=AGENTE_IA_ID,     # retorno do agente aparece como da propria VALOIS.IA
