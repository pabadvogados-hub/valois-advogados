...
Usa a VALOIS.IA (Claude) para analisar documentos e gerar a peca completa.
"""
import anthropic
import json
import os
...
# ============================================================
# DADOS DO ESCRITORIO (lidos do ambiente, com defaults VALOIS_ADVOGADOS)
# ============================================================
ESCRITORIO_NOME = os.getenv('ESCRITORIO_NOME', 'Welington Valois Advogados Associados')
ESCRITORIO_ADVOGADO = os.getenv('ESCRITORIO_ADVOGADO', 'Dr. Welington Dias Valois')
ESCRITORIO_OAB = os.getenv('ESCRITORIO_OAB', 'OAB/ES 34.912')
ESCRITORIO_CIDADE = os.getenv('ESCRITORIO_CIDADE', 'Iúna/ES')
...
def gerar_peticao_inicial(classificado: dict) -> dict:
    ...
    print(f"  Contexto: {chars_usados:,} caracteres enviados para a VALOIS.IA")
    ...
    system_prompt = f'''Voce e VALOIS.IA, advogada senior com 20 anos de experiencia pratica em direito processual trabalhista, e trabalha em conjunto com o(a) advogado(a) responsavel do {ESCRITORIO_NOME}, {ESCRITORIO_ADVOGADO}, inscrito(a) na {ESCRITORIO_OAB}. Voces formam um time vitorioso.
...
[qualificacao completa do reclamante com todos os dados pessoais disponíveis - justificado, recuo 1,25cm]
...
[Narrativa cronologica dos fatos baseada EXCLUSIVAMENTE nos documentos fornecidos - justificado, recuo 1,25, Espacamento 1,15]
...
4. FORMATACAO: corpo JUSTIFICADO, recuo 1,25, Espacamento 1,15. Titulos de secao com numeral alfanumerico (I., II., III., IV., V., A), B)) sao NEGRITO + JUSTIFICADO - NUNCA centralizados. So centralizar: enderecamento, nome da acao, encerramento, data e assinatura.
...
RETORNE APENAS O JSON. Nao inclua texto, introducoes ou blocos ```json markdown.'''
    ...
    print("  Enviando documentos para analise da VALOIS.IA (Claude)...")
