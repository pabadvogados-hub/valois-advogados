# Deploy do Agente VALOIS.IA na VPS (Hostinger / Ubuntu)
...
> O `.git` de producao e ISOLADO (so a pasta `VALOIS_ADVOGADOS`). Nenhum segredo
...
> A VPS de producao do Welington Valois Advogados Associados AINDA NAO foi provisionada.
...
- **Projeto na VPS:** `/home/deploy/valois-advogados` | virtualenv em `.venv`.
- **Clone do repo privado:** via **deploy key** read-only (`~/.ssh/github_deploy` na VPS).
- **Servico:** systemd `valois-agente` (enabled, Restart=always), uvicorn na porta **8787**.
...
```bash
# logs em tempo real
journalctl -u valois-agente -f
# status / restart
sudo systemctl status valois-agente
sudo systemctl restart valois-agente
# deploy de nova versao
cd /home/deploy/valois-advogados && git pull
...
- Repositorio no GitHub (privado) com o codigo — `pabadvogados-hub/valois-advogados`.
...
Repositorio **privado** ja criado: `pabadvogados-hub/valois-advogados`. Na pasta
do projeto:

```bash
git remote add origin git@github.com:pabadvogados-hub/valois-advogados.git
git push -u origin master
```

(Se usar HTTPS no lugar de SSH: `https://github.com/pabadvogados-hub/valois-advogados.git`
...
```bash
cd ~
git clone git@github.com:pabadvogados-hub/valois-advogados.git valois-advogados
cd valois-advogados
```
...
```bash
bash deploy/setup_vps.sh ~/valois-advogados
```
...
```powershell
# rode na maquina LOCAL, dentro da pasta VALOIS_ADVOGADOS
scp config/.env                 deploy@SEU_IP:~/valois-advogados/config/.env
scp config/credentials.json     deploy@SEU_IP:~/valois-advogados/config/credentials.json
scp config/oauth_credentials.json deploy@SEU_IP:~/valois-advogados/config/oauth_credentials.json
scp config/token.json           deploy@SEU_IP:~/valois-advogados/config/token.json
scp config/timbrado_modelo.docx deploy@SEU_IP:~/valois-advogados/config/timbrado_modelo.docx
```
...
sudo cp deploy/valois-agente.service /etc/systemd/system/valois-agente.service
# confira User= e os caminhos dentro do arquivo (default: usuario 'deploy', ~/valois-advogados)
sudo systemctl daemon-reload
sudo systemctl enable --now valois-agente
sudo systemctl status valois-agente --no-pager
```
...
Quando quiser expor com dominio e SSL (ex.: `agente.SEU-DOMINIO.com.br` —
